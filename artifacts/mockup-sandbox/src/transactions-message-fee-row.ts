...
function reconcile(list:HTMLElement){const rows=Array.from(list.children).filter((e):e is HTMLButtonElement=>e instanceof HTMLButtonElement&&findAmountBox(e)!==undefined);if(!rows.length)return;let b=roundMoney(currentAccountBalance());for(const row of [...rows].reverse()){b=roundMoney(b+signedAmount(row));setRowBalance(row,b);}}
